...
class ReLU:
    ...
    def forward(self, z):
        ...
        if dim == 1:
            ...
            batch_size, channel_num, height, width = z.shape
            for curBat in range(0, batch_size):
                for curCh in range(0, channel_num):
                    for h in range(0, height):
                        for w in range(0, width):
                            if z[curBat][curCh][h][w] < 0:
                                self.zero_mask[curBat][curCh][h][w] = 1
                                out[curBat][curCh][h][w] = 0
                            else:
                                out[curBat][curCh][h][w] = z[curBat][curCh][h][w]
    ...
    def backward(self, d_prev):
        ...
        if dim == 1:
            ...
            batch_size, channel_num, height, width = d_prev.shape
            mask_channel = self.zero_mask.shape[1]
            for curBat in range(0, batch_size):
                for curCh in range(0, channel_num):
                    for mc in range(0, mask_channel):
                        for h in range(0, height):
                            for w in range(0, width):
                                if self.zero_mask[curBat][mc][h][w] != 1:
                                    dz[curBat][curCh][h][w] = d_prev[curBat][curCh][h][w]
        '''
        print("<relu backward>")
        print("d_prev")
        print(d_prev)
...
class ConvolutionLayer:
    ...
    def backward(self, d_prev):
        ...
        ** [HINT] **
        ...
        for curBat in range(0, batch_size):
            for curOut in range(0, out_channel):
                for curIn in range(0, in_channel):
                        for i in range(0, conv_height, stride):
                            for j in range(0, conv_width, stride):
                                for a in range(0, kernel_size):
                                    for b in range(0, kernel_size):
                                        self.dW[curOut][curIn][a][b] += self.x[curBat][curIn][a+i][b+j] * d_prev[curBat][curOut][i][j]
                                        dx[curBat][curIn][a+i][b+j] += self.W[curOut][curIn][a][b] * d_prev[curBat][curOut][i][j]
                                self.db[curOut] += d_prev[curBat][curOut][i][j]
    ...
    def zero_pad(self, x, pad):
        ...
        _, _, new_height, new_width = padded_x.shape

        for curBat in range(0, batch_size):
            for curCh in range(0, in_channel):
                for i in range(0, new_height):
                    for j in range(0, new_width):
                        if i < pad or i >= new_height - pad or j < pad or j >= new_width - pad:
                            padded_x[curBat][curCh][i][j] = 0
                        else:
                            padded_x[curBat][curCh][i][j] = x[curBat][curCh][i - pad][j - pad]
...
class MaxPoolingLayer:
    ...
    def forward(self, x):
        ...
        if self.kernel_size != self.stride:
            print("kernel size and stride are different.")
            return max_pool
        ...
        max_pool = np.zeros((batch_size, channel, nh, nw))

        for curBat in range(0, batch_size):
            for curCh in range(0, channel):
                for i in range(0, nh):
                    for j in range(0, nw):
                        max = x[curBat][curCh][i * k][j * k]
                        mx = i * k
                        my = j * k
                        for a in range(0, k):
                            for b in range(0, k):
                                temp = x[curBat][curCh][i * k + a][j * k + b]
                                if temp >= max:
                                    max = temp
                                    self.mask[curBat][curCh][mx][my] = 0
                                    mx = i * k + a
                                    my = j * k + b
                                    self.mask[curBat][curCh][mx][my] = 1
                        max_pool[curBat][curCh][i][j] = max
    ...
    def backward(self, d_prev=1):
        ...
        for curBat in range(0, batch):
            for curCh in range(0, self.mask.shape[1]):
                for h in range(0, height):
                    for w in range(0, width):
                        for a in range(0, k):
                            for b in range(0, k):
                                if self.mask[curBat][curCh][h*k+a][w*k+b] == 1:
                                    d_max[curBat][curCh][h*k+a][w*k+b] = d_prev[curBat][curCh][h][w]
...
class FCLayer:
    ...
    def backward(self, d_prev):
        ...
        # dW
        trans = np.transpose(d_prev)
        self.dW = np.matmul(trans, self.x)
        self.dW = np.transpose(self.dW)
...
class SoftmaxLayer:
    ...
    def ce_loss(self, y_hat, y):
        ...
        batch_size, class_num = y.shape

        self.loss = 0.
        for curBat in range(0, batch_size):
            self.loss -= np.sum(y[curBat] * np.log(y_hat[curBat] + eps))
