...
class Decision_Tree():
    ...
    def Find_Best_Feature(self, df):
        ...
        for idx, h in enumerate(input_feature):
                ...
                for kind in kinds:
                    ...
                    for value in df.values:
                        if value[idx] == kind:
                            ...
                            y.append(value[-1])
                    p = n / size
                    impurity += impurity_func(y, self.criterion) * p
            ...
            # Numeric Feature Case
                ...
                p1 = n1 / size
                p2 = 1 - p1
                impurity += impurity_func(y1, self.criterion) * p1
                impurity += impurity_func(y2, self.criterion) * p2
            # =====================================================
    ...
    def set_leaf_node(self, node):
        ...
        # print('\t' * node.depth, 'parent: ', self.branch_name, '\t ', 'Leaf node \t Depth: ', node.depth, '\tLabel: ',
        #       node.label)
        return
