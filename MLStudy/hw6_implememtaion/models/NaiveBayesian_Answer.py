...
class Gaussian_NaiveBayesian():
    ...
    # P(Yes), P(No)
      # ======     Edit this    ======
        n = len(self.df)
        values = Y_data.values.tolist()
        self.prob_yes = values.count('Yes') / n
    ...
    def set_likelihood_prob(self, data, likelihood):
        ...
        for idx, f in enumerate(input_feature):
            ...
            if idx in self.Category_feature_idx:
                ...
                pass
                likelihood['%s_mean' % f] = np.round(np.mean(column_data), 6)
                likelihood['%s_std' % f] = np.round(np.std(column_data), 6)
        # ================================================
        return
        ...
        for idx, f in enumerate(feature):
            ...
            if idx in self.Category_feature_idx:
                ...
                if type(val) == np.float64 or type(val) == np.float32:
                    val = str(int(val))
                cur_like = likelihood['%s = %s' % (f, val)]
                posterior = cur_like * prior
            else:
                mean = likelihood['%s_mean' % f]
                std = likelihood['%s_std' % f]
                cur_like = Gaussian_prob(val, mean, std)
                posterior = cur_like * prior
        # ===============================================================
        return posterior
